...
export default function Home() {
    ...
    useEffect(()=> {
        const filteredObj = Object.keys(oldList).length ? Object.keys(oldList).filter((key) => key.includes(searchKeyWord)).reduce((obj,key) => {
            return Object.assign(obj, {[key] : oldList[key]});
        },{}) : {} ;
    ...
    return (
        <div className="">
            ...
            <ul className="grid gap-x-0 gap-y-0 sm:grid-cols-1 md:grid-cols-3 lg:grid-cols-5 mb-20 " >
                {
                    !loading && error && <h1 className="color-red">Error ocurred: {error}</h1>
                }
                {
                        ...
                        <li key={`${key}${value}`} className="shadow-md sm:h-1/8 dark:bg-gray-800 dark:border-gray-700 p-5 cursor-pointer odd:bg-sky-500 even:bg-sky-700">
                            <NavLink to={`detail?product_type=${key}`} className="">
                                <div href="#" className="flex">
                                    <img className="p-8 rounded-t-lg fill-white w-64 flex-1" src={require(`../assets/${key}.svg`)} alt="product image" />
