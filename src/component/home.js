    ...
    return (
        <div className="p-5">
            <div className='flex flex-col justify-center items-center p-5 '>
                <img src={Background} alt="Image of make tool -z-10" className='w-1/4'/>
                <div className='absolute top-30 w-1/2'>
                    ...
                    <div className="mb-3 xl:w-96 flex w-full">
                        <div className="input-group relative flex items-stretch w-full mb-4">
                            <input type="search" className="form-control relative flex-auto min-w-0 block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none opacity-75" placeholder="Filter by category name" aria-label="Search" aria-describedby="button-addon2"/>
            ...
            <ul className="grid gap-x-8 gap-y-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 p-5 mb-20 " >
                {
                    !loading && Object.entries(category).map(([key, value]) => (
                        <li style={{ backgroundColor: '#00000040' }}  key={`${key}${value}`} className="max-w-sm rounded-lg shadow-md dark:bg-gray-800 dark:border-gray-700 p-5 cursor-pointer">
                            <NavLink to={`detail?product_type=${key}`} className="">
                                <div href="#" className="flex">
                                    <img className="p-8 rounded-t-lg color-white w-64 flex-1" src={require(`../assets/${key}.svg`)} alt="product image" />
                                    <p className='text-white'><MdArrowForward /></p>
                                </div>
                                <div className='flex justify-end items-center md:gap-5 lg:flex-col'>
                                    <h3 className='text-center font-medium leading-8 text-white'>{formatString(key)}</h3>
                                    <p className='text-right font-medium leading-8 text-white'>{value}</p>
                                </div>
                            </NavLink>
                            <Outlet />
                        </li>
